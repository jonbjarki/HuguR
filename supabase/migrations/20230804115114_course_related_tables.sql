...
create table "public"."units"(
    ...
    "week" smallint not null, -- TODO modules might have different names, create a modules table
    "course_id" bigint not null,
...
CREATE OR REPLACE FUNCTION public.set_unit_translation_key()
RETURNS TRIGGER AS $$
BEGIN
    NEW.translation_key := 'unit' || NEW.id;
    RETURN NEW;
END;
$$ LANGUAGE plpgsql security definer;
CREATE TRIGGER set_unit_translation_key_trigger
    BEFORE INSERT ON public.units
    FOR EACH ROW
    EXECUTE FUNCTION public.set_unit_translation_key();
